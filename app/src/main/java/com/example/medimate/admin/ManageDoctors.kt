...
fun ManageDoctors(navController: NavController) {
    ...
    OutlinedTextField(value = specialisation, onValueChange = { specialisation = it }, label = { Text("Specialisation") }, modifier = Modifier.fillMaxWidth())
    ...
    Column(
        ...
        Button(onClick = {
            val doctor = Doctor(id=adminDAO.generateDoctorId(),name=name, surname = surname,
                email = email,
        ...
        coroutineScope.launch {
            try {
                ...
                Toast.makeText(context, "Failed to add a doctor", Toast.LENGTH_SHORT).show()
            }
        }
    }, modifier = Modifier.padding(top = 8.dp), enabled = name.isNotBlank() && surname.isNotBlank() && email.isNotBlank()){
